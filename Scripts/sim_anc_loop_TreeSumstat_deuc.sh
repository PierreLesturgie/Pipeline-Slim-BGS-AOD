...
### For example: the simulation run on the 17th CPU of the 3rd node will have the tag 173. Be careful with the correspondance with divdel scnearios
...
#### This "$prefix/${file}_for_loop.sh" script runs a unique simulation (SLIM) and estimation (GTS). 
#### It is used by the "higher" level script "$prefix/${file}.sh" that will run this script
#### for each of the 128 cpus in each of the 8 nodes
#### resulting in 1024 simulations. 

TASK_ID=\$SLURM_ARRAY_TASK_ID
tpernode=128
task=\$1

slim -l 0 -s 9\${TASK_ID}35\${task}2 -d rep=\${task}\${TASK_ID} ${file} 
#gts stats -t ${prefix}_\${task}\${TASK_ID}_ancestralTreeSeq -Nanc 1000 -r 10 -R ${reco} -D ../../scenario_anc.txt -G ../../genome.txt
#gts summary --tag ${prefix}_\${task}\${TASK_ID}_ancestralTreeSeq -r 10 -G ../../genome.txt 
#rm ${prefix}_\${task}\${TASK_ID}_ancestralTreeSeq_stats_*.detailed
...
#!/bin/bash
#SBATCH --job-name=simanc_${prefix}_run
...
source ~/.bashrc 
conda activate /projects/F202500293CPCAA2/ablanckaert/slim ### This is an environment with slim v5 
