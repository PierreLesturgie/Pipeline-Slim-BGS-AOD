...
do
    ...
    #mkdir $prefix
    inputfile1=ms_${prefix}_\${task}\${TASK_ID}_G10100.ms
    inputfile2=ms_${prefix}_\${task}\${TASK_ID}_G11000.ms
    inputfile3=ms_${prefix}_\${task}\${TASK_ID}_G12000.ms
...
cat > $prefix/${file}_R_loop.sh << EOF
#!/bin/bash
...
Rscript /projects/F202500293CPCAA2/ablanckaert/process_file_alex.R ${inputfile1} ${outputfile}
Rscript /projects/F202500293CPCAA2/ablanckaert/process_file_alex.R ${inputfile2} ${outputfile}
Rscript /projects/F202500293CPCAA2/ablanckaert/process_file_alex.R ${inputfile3} ${outputfile}

# slim -l 0 -s 9\${task}51\${task}2 -d rep=\${task}\${TASK_ID} ${file} 
...
cat > $prefix/${file}_R.sh << EOF
#!/bin/bash
#SBATCH --job-name=Rdivdel_${prefix}_run
