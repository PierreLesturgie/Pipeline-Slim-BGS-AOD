...
#' GET_DAFI
#' Calculate DAFI from genotype matrix
#' @param geno numeric matrix with nsites x nind coded as 0, 1 or 2 genotypes (0 homo ancestral, 1 het,2 homo derived) without missing data!
#'             NO MISSING DATA allowed!
#'             using formula in eq. 3 of Poyet et al 2018 https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6177262/
#' @return vector with value of DAFi for each individual in the matrix
#'
#' @examples
get_dafi <- function(geno) {
  # get the number of derived alleles per individual
  nderivedalleles <- colSums(geno)
  
  # get the total number of sites polymorphic in a sample of individuals
  stot <- nrow(geno)
  
  # DAFI
  dafi <- nderivedalleles/(2*stot)
  dafi
}
## END
